...
# Section priority order for trimming (lower = higher priority, kept longest)
...
def trim_to_budget(report_text: str, budget_tokens: int) -> str:
    """Trim a report to fit within *budget_tokens* by dropping low-priority sections.

    Sections are dropped from the bottom of the priority list first.
    The header (everything before the first '## ') and the footer (after last
    '---') are always kept.
    """
    if _estimate_tokens(report_text) <= budget_tokens:
        return report_text

    # Split report into header + sections
    ...
    for line in lines:
        ...
        if in_footer:
            ...
            continue
        if line.startswith("## ") and not header_lines or (sections or current_lines):
            if current_heading or current_lines:
                sections.append((current_heading, current_lines))
            current_heading = line
            current_lines = []
        elif not sections and not current_lines and not current_heading:
    ...
    # Build priority index for sections
    def _priority(heading: str) -> int:
        for i, phead in enumerate(_SECTION_PRIORITY):
            if heading.startswith(phead):
                return i
        return len(_SECTION_PRIORITY)

    sections_sorted = sorted(sections, key=lambda s: _priority(s[0]))

    # Drop lowest-priority sections until within budget
    kept = list(sections_sorted)
    while kept and _estimate_tokens(
        "\n".join(header_lines) + "\n".join(
            h + "\n" + "\n".join(ls) for h, ls in kept
        ) + "\n".join(footer_lines)
    ) > budget_tokens:
        kept.pop()  # drop last (lowest priority)

    # Reassemble in original order (re-sort by original position)
    kept_headings = {h for h, _ in kept}
    ordered = [s for s in sections if s[0] in kept_headings]

    parts = ["\n".join(header_lines)]
    for heading, sec_lines in ordered:
    ...
    trimmed += f"\n\n> *Report trimmed to ~{trimmed_tokens:,} tokens (budget: {budget_tokens:,})*\n"
    return trimmed
...
def generate(
    ...
    if token_budget > 0:
        report = trim_to_budget(report, token_budget)
